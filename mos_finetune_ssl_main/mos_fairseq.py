...
class MyDataset(Dataset):
    def __init__(self, wavdir, mos_list):
        ...
        #     wavname = parts[0] ; print('\nwavname =', wavname, '\n')
        #     mos = float(parts[1])
        #     self.mos_lookup[wavname] = mos
        # self.wavdir = wavdir
        # self.wavnames = sorted(self.mos_lookup.keys())
        
        self.score = float(3.0)
        self.wavdir = wavdir # wavdir is np.ndarray
        self.wavname = mos_list # wavlist = mos_list = wavname = 'mydir'

    def __getitem__(self, idx):
        # if isinstance(self.wavdir, np.ndarray): # self.wavdir is np.ndarray
        wavname = self.wavname # wavname = 'mydir'
        wavpath = None
        wav = self.wavdir # wavdir is np.ndarray
        score = self.score
        # elif isinstance(self.wavdir, list): # self.wavdir is np.ndarray List
        #     wavname = self.wavnames[idx]
        #     wavpath = None
        #     wav = torch.tensor([int(char) for char in wavname], dtype=torch.float32)
        #     score = self.mos_lookup[wavname]
        # elif self.wavdir.split('.')[-1] == 'wav': # self.wavdir is .wav
        #     wavname = self.wavnames[idx]
        #     wavpath = self.wavdir
        #     wav = torchaudio.load(wavpath)[0]
        #     score = self.mos_lookup[wavname]
        # else: # self.wavdir is .wav directory
        #     wavname = self.wavnames[idx]
    ...
    def collate_fn(self, batch):  ## zero padding
        ...
        # elif isinstance(self.wavdir, list): # self.wavdir is np.ndarray List
        #     max_len = max(wavs, key = lambda x : x.shape[1]).shape[1]
        # elif self.wavdir.split('.')[-1] == 'wav': # wavs is .wav
        #     max_len = max(wavs, key = lambda x : x.shape[1]).shape[1]
        #     # print(wavs[0]) # tensor(1*66150), wavs 的 wav
        #     # print(wavs[0].shape[0]) # 1
        #     # print(wavs[0].shape[1]) # 66150
        #     # print(max(wavs, key = lambda x : x.shape[1])) # tensor(1*66150), wavs 裡最大的 wav
        #     # print(max(wavs, key = lambda x : x.shape[1]).shape[0]) # 1
        #     # print(max(wavs, key = lambda x : x.shape[1]).shape[1]) # 66150
        # else: # wavs is .wav directory
        #     max_len = max(wavs, key = lambda x : x.shape[1]).shape[1]
        output_wavs = []
        for wav in wavs:
            # if isinstance(self.wavdir, np.ndarray): # self.wavdir is np.ndarray
