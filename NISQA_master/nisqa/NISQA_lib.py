...
class SpeechQualityDataset(Dataset):
    ...
    def __init__(
        ...
        self.df = df
        self.df_con = df_con
        self.data_dir = data_dir #### assume 'data_dir' is waveform, not path 
    ...
    def _load_spec(self, index):
            ...
            file_path = self.data_dir #### assume 'data_dir' is waveform, not path 
            ...
            spec = get_librosa_melspec(
                file_path, #### assume 'data_dir' is waveform, not path 
                sr = self.ms_sr,
                n_fft=self.ms_n_fft,
            ...
            if self.double_ended:
                spec_ref = get_librosa_melspec(
                    file_path_ref, #### assume 'data_dir' is waveform, not path 
                    sr = self.ms_sr,
                    n_fft=self.ms_n_fft,
                    hop_length=self.ms_hop_length,
                    win_length=self.ms_win_length,
...
def get_librosa_melspec(
    ...
    try:
        if ms_channel is not None:
            ...
            y, sr = file_path, 16_000 #### assume 'data_dir' is waveform, not path 
            if len(y.shape)>1:
                y = y[ms_channel, :]
        else:
