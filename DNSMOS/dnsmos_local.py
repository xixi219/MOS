...
def main(args):
    ...
    rows = []
    clips = []   
    if isinstance(args.testset_dir, np.ndarray): clips = [(args.testset_dir, args.sampling_rate)] # clips is np.ndarray tuple
    elif isinstance(args.testset_dir, list) and all(isinstance(array, np.ndarray) for array in args.testset_dir):
        clips = [(array, args.sampling_rate) for array in args.testset_dir] # clips is np.ndarray tuple List
    else: # clips is .wav or .wav List
        clips = glob.glob(os.path.join(args.testset_dir)) if args.testset_dir.endswith('.wav') else glob.glob(os.path.join(args.testset_dir, "*.wav"))
